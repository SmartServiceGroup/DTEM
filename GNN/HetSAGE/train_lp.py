...
from model import LinkPredictionScorer_V2 as LinkPredictionScorer
...
补充关于 LP ER 训练方法的实验，这部分数据的命名方式是：
L = 只有 LP
E = 只有 ER
LE = 先 LP 再 ER （这是最开始的版本）   
EL = 先 ER 再 LP
L/E = 两任务同时训练

这就不区分预训练/训练, GNN训练好的模型都放在trained下面, dump出来的嵌入还是放在node_embedding下

训练脚本使用 train_lp.py, train_er.py
没有什么区别，除了是否加载预训练模型这个细节。
'''
...
class ParameterNamespace():
    def __init__(self, special_suffix):
        ...
        # Used if training on pretrained model
        self.pretrain_model_path = "./trained/Comparison_LP_ER/E/model.bin"
...
if __name__ == "__main__":
    ...
    # If not training on pretained model, disable this line
