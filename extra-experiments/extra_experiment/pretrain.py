...
ALL_CONFIGS = {
    'large': {
        'experiment_name': 'HetSAGE_on_LARGE_community',
        'graph_path': 'data_preprocess/full_graph/large.bin',
    },
    'middle': {
        'experiment_name': 'HetSAGE_on_MIDDLE_community',
        'graph_path': 'data_preprocess/full_graph/middle.bin',
    },
    'little': {
        'experiment_name': 'HetSAGE_on_LITTLE_community',
        'graph_path': 'data_preprocess/full_graph/little.bin',
    },
}


CONFIG = ALL_CONFIGS['little']
EXPERIMENT_NAME = CONFIG['experiment_name']
SPECIAL_SUFFIX  = CONFIG['experiment_name']
...
def train(model, lp_scorer, train_dataloader, eval_dataloader, test_dataloader, node_feats, epochs, lr=0.001, start_epoch=0):
    ...
    print(datetime.now())
    log.info(f'Start training. {start_epoch = }')
...
# Something went wrong during training (i.e. CUDA OOM)
# We need to rerun the training from a saved state dict
# mostly copied from the training code above

RECOVER_ALL_CONFIGS = {
    'large': {
        'state_dict_flp': {
            'model':     'checkpoint/HetSAGE_on_LARGE_community_2025,01,20,11,52,22/model_in_epoch27',
            'lp_scorer': 'checkpoint/HetSAGE_on_LARGE_community_2025,01,20,11,52,22/lp_scorer_in_epoch27'
        },
        'epoch': 27,
    }, 
    'little': {
        'state_dict_flp': {
            'model':     'checkpoint/HetSAGE_on_LITTLE_community_2025,01,21,05,06,23/model_in_epoch28',
            'lp_scorer': 'checkpoint/HetSAGE_on_LITTLE_community_2025,01,21,05,06,23/lp_scorer_in_epoch28'
        },
        'epoch': 28,
    },
}
RECOVER_CONFIG = RECOVER_ALL_CONFIGS['little']
...
if __name__ == "__main__":
    ...
    model = HetSAGE(
        ...
        num_hidden_layers=pn.num_hidden_layers,
        feat_drop=pn.feat_drop)
    model.load_state_dict(torch.load(RECOVER_CONFIG['state_dict_flp']['model']))
    model = model.to(pn.device)
    log.info('Model loaded')

    lp_scorer = LinkPredictionScorer(in_features=pn.out_feats)
    lp_scorer.load_state_dict(torch.load(RECOVER_CONFIG['state_dict_flp']['lp_scorer']))
    lp_scorer = lp_scorer.to(pn.device)
    log.info('LinkPredictionScorer loaded')
    ...
    train(
        ...
        pn.epochs,
        pn.lr,
        start_epoch=RECOVER_CONFIG['epoch']
    )
