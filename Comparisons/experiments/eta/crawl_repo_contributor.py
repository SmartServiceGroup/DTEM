...
def crawl_contributor_info():
    ...
    skipped_repos = set()
    with open("./rawdata/repo_contribution.txt", "r", encoding="utf-8") as inf:
        for line in inf:
            repo_name, _ = line.strip().split("\t")
            skipped_repos.add(repo_name.lower())
    
    repo_contri_outf = open("./rawdata/repo_contribution.txt", "a+", encoding="utf-8")
    
    with open("./rawdata/valid_repos.txt", "r", encoding="utf-8") as inf:
        for line in inf:
